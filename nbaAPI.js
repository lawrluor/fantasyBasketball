// Source: https://github.com/kevinn03/nba_api/blob/master/utils/news_helper.js
// kevinn03's nba_api
...
const getData = async (website) => {
  ...
  try {
    ...
    const res = await axios.get(website.address, options);
    const html = res.data;
    const $ = cheerio.load(html);

    $(website.selector, html).each(function () {
      console.log('Getting data from:', website.address);
      if (!website.address) {
        return;
      }
      ...
      console.log('originalArticles', originalArticles);
    });
